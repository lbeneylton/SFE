...
class Calendar(ttk.Frame):
    ...
    def __init__(self, master=None, **kw):
        ...
        ttk.Frame.__init__(self, master, class_=classname,
                           cursor=curs, name=name)
        ...
        firstweekday = kw.pop('firstweekday', '')
        if firstweekday not in ["monday", "sunday"]:
            raise ValueError(
                "'firstweekday' option should be 'monday' or 'sunday'.")
        ...
        date_pattern = self._get_date_pattern(
            kw.pop("date_pattern", "short"), locale)
        ...
        if self._textvariable is not None:
            ...
            if (("month" in kw) or ("year" in kw)) and ("day" not in kw):
                ...
                year = kw.pop('year', today.year)
                try:
                    self._sel_date = self.date(
                        year, month, day)  # selected day
                except ValueError:
                    self._sel_date = None

        # (year, month) displayed by the calendar
        ...
        if maxdate is not None:
            if isinstance(maxdate, self.datetime):
                ...
                raise TypeError(
                    "expected %s for the 'maxdate' option." % self.date)
        if mindate is not None:
            if isinstance(mindate, self.datetime):
                mindate = mindate.date()
            elif not isinstance(mindate, self.date):
                raise TypeError(
                    "expected %s for the 'mindate' option." % self.date)
        ...
        dis_active_bg = self.style.lookup(
            'TEntry', 'selectbackground', ('disabled',))
        ...
        # --- init calendar
        # --- *-- header: month - year
        self._header = ttk.Frame(
            self, style='main.%s.TFrame' % self._style_prefixe)
        ...
        f_year = ttk.Frame(self._header, style='main.%s.TFrame' %
                           self._style_prefixe)
        ...
        for i, day in enumerate(self._cal.iterweekdays()):
            ...
            self._week_days.append(ttk.Label(self._cal_frame,
                                             ...
                                             anchor="center",
                                             text=d, width=4))
            self._week_days[-1].grid(row=0, column=i + 1,
                                     sticky="ew", pady=(0, 1))
        ...
        for i in range(1, 7):
            ...
            for j in range(1, 8):
                ...
                label.grid(row=i, column=j, padx=(0, 1),
                           pady=(0, 1), sticky="nsew")
                if selectmode == "day":
    ...
    def __setitem__(self, key, value):
        if key not in self._properties:
            ...
            if key == "selectmode":
                ...
                self._properties['date_pattern'] = self._get_date_pattern(
                    "short", value)
                for i, l in enumerate(self._week_days):
                    l.configure(text=self._day_names[i])
                self._header_month.configure(
                    text=self._month_names[self._date.month].title())
                ...
                self.style.configure('R.%s.TButton' %
                                     self._style_prefixe, arrowsize=size)
                self.style.configure('L.%s.TButton' %
                                     self._style_prefixe, arrowsize=size)
            elif key == "normalbackground":
                self.style.configure('cal.%s.TFrame' %
                                     self._style_prefixe, background=value)
                ...
                self.style.configure('normal.%s.TLabel' %
                                     self._style_prefixe, foreground=value)
                ...
                self.style.configure('sel.%s.TLabel' %
                                     self._style_prefixe, background=value)
                ...
                self.style.configure('we.%s.TLabel' %
                                     self._style_prefixe, background=value)
                self.style.configure('we_om.%s.TLabel' %
                                     self._style_prefixe, background=value)
                ...
                self.style.configure('main.%s.TFrame' %
                                     self._style_prefixe, background=value)
                self.style.configure('main.%s.TLabel' %
                                     self._style_prefixe, background=value)
                ...
                self.style.configure('R.%s.TButton' %
                                     self._style_prefixe, arrowcolor=value)
                self.style.configure('L.%s.TButton' %
                                     self._style_prefixe, arrowcolor=value)
                self.style.configure('main.%s.TLabel' %
                                     self._style_prefixe, foreground=value)
            ...
            if len(weekenddays) != 2:
                ...
                for d in weekenddays:
                    if d not in range(1, 8):
                        raise ValueError(
                            "weekenddays should contain integers between 1 and 7.")
        except TypeError:
            ...
            if not date:
                ...
                try:
                    ...
                    if self._sel_date is None:
                        ...
                        self._textvariable.set(
                            self.format_date(self._sel_date))
    ...
    def _setup_style(self, event=None):
        ...
        self.style.configure('cal.%s.TFrame' %
                             self._style_prefixe, background=bc)
        self.style.configure('main.%s.TLabel' %
                             self._style_prefixe, background=bg, foreground=fg)
        ...
        self.style.map('%s.TButton' % self._style_prefixe,
                       background=[('active', '!disabled', active_bg),
                                   ('disabled', dis_bg)],
    ...
    def _display_days_without_othermonthdays(self):
        ...
        week_days[self['weekenddays'][0] -
                  1] = 'we.%s.TLabel' % self._style_prefixe
        week_days[self['weekenddays'][1] -
                  1] = 'we.%s.TLabel' % self._style_prefixe
                ...
                if day_number:
                    ...
                    if date in self._calevent_dates:
                        ...
                        if i >= 0:
                            ...
                            label.configure(style='tag_%s.%s.TLabel' %
                                            (tag, self._style_prefixe))
                        text = '\n'.join(
                            ['➢ {}'.format(self.calevents[ev]['text']) for ev in ev_ids])
    ...
    def _display_selection(self):
        ...
        if self._sel_date is not None:
            ...
            if w is not None:
                ...
                if label.cget('text'):
                    label.configure(style='sel.%s.TLabel' %
                                    self._style_prefixe)
        ...
        if w is not None:
            ...
            if month == date.month:
                ...
                if d in week_end:
                    ...
                    self._calendar[w][d].configure(
                        style='normal_om.%s.TLabel' % self._style_prefixe)
    ...
    def _remove_selection(self):
        ...
        if self._sel_date is not None:
            if self._sel_date in self._calevent_dates:
                ...
                if w is not None:
                    week_end = [
                        0, 6] if self['firstweekday'] == 'sunday' else [5, 6]
    ...
    def calevent_create(self, date, text, tags=[]):
        ...
        if not isinstance(date, Calendar.date):
            raise TypeError("date option should be a %s instance" %
                            (Calendar.date))
        if self.calevents:
    ...
    def calevent_configure(self, ev_id, **kw):
        ...
        try:
            ...
            if kw:
                raise KeyError(
                    'Invalid keyword option(s) %s, valid options are "text", "tags" and "date".' % (kw.keys(),))
            else:
                if text is not None:
                    ev['text'] = str(text)
                if tags is not None:
                ...
                if date is not None:
                    ...
                    if not isinstance(date, Calendar.date):
                        raise TypeError(
                            "date option should be a %s instance" % (Calendar.date))
                    old_date = ev['date']
    ...
    def calevent_raise(self, ev_id, above=None):
        ...
        try:
            ...
            if above is None:
                ...
                if above not in evs:
                    raise ValueError(
                        "event %s does not exists on %s" % (above, date))
                else:
                    evs.remove(ev_id)
    ...
    def calevent_lower(self, ev_id, below=None):
        ...
        try:
            ...
            if below is None:
                ...
                if below not in evs:
                    raise ValueError(
                        "event %s does not exists on %s" % (below, date))
                else:
                    evs.remove(ev_id)
    ...
    def _tag_initialize(self, tag):
        ...
        self.style.configure('tag_%s.%s.TLabel' %
                             (tag, self._style_prefixe), **props)
